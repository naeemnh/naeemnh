...
const Window = ({
  ...
  const { windows, focusWindow, updateWindowPosition, setWindows } =
    useWindow();
  const [currentBounds, setCurrentBounds] = useState<ControlPosition>(
  ...
  const closeWindow = useCallback(
    function (e: MouseEvent) {
      ...
      setWindows((windows) =>
        windows.filter((_, index) => index !== windowIndex),
      );
      e.stopPropagation();
    },
    [window],
  ...
  }, [mediaQueryList, setIsSmallScreen]);
  ...
  const handlePosition = (_: DraggableEvent, ui: DraggableData) => {
    setCurrentBounds({
      x: currentBounds.x + ui.deltaX,
      y: currentBounds.y + ui.deltaY,
    });
  };
  ...
  return (
    <>
      <Draggable
        ...
        position={isSmallScreen ? defaultPosition : position}
        onStart={handleFocus}
        handle="div.draggable"
        onDrag={handlePosition}
        onStop={handleWindowPosition}
      >
        <div
          ...
          style={{
            zIndex: props.zIndex,
            display:
              isSmallScreen && props.zIndex < windows.length + 9
                ? "none"
                : "block",
          }}
        >
